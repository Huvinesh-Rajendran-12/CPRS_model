...
from postgres import get_group_details

client_data = pd.read_csv("./data/clients.csv", skipinitialspace=True)

student_group_data = get_group_details(1)
print(student_group_data.columns)
print(student_group_data)
# prepare client data
client_data["Requirements"] = (
    client_data["Project brief "].map(str)
    + " "
    + client_data["Project Background"]
    + " "
    + client_data["Requiements"]
)
...
student_group_data["details"] = student_group_data["details"].apply(clean_txt)
...
stud_group_rec = tfidf_vect.transform(student_group_data["details"])
cos_stud_group_rec = list(
    map(lambda x: cosine_similarity(stud_group_rec, x), tfidf_comb)
)
...
print(get_recommendation(1, top10_stud_group_rec, client_data, list_scores))
